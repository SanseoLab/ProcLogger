...
UNICODE_STRING g_PolicyKeyArray[] = {
	RTL_CONSTANT_STRING(L"\\REGISTRY\\MACHINE\\System\\CurrentControlSet\\Services\\ProcLogger"),
	RTL_CONSTANT_STRING(L"\\REGISTRY\\MACHINE\\System\\ControlSet001\\Services\\ProcLogger"),
	RTL_CONSTANT_STRING(L"\\REGISTRY\\MACHINE\\System\\ControlSet002\\Services\\ProcLogger"),
	RTL_CONSTANT_STRING(L"\\REGISTRY\\MACHINE\\System\\ControlSet003\\Services\\ProcLogger")
};
...
BOOLEAN CheckPolicy(PUNICODE_STRING KeyFullPath) {
	...
	if (Matched) {
		DbgPrint("[ ProcLogger ] [ RegMonitor ] pid(%x) and tid(%x) Block %wZ\n",
...
NTSTATUS RegPreOpenKey(PVOID RootObject, PUNICODE_STRING CompleteName)
{
	...
	if (RootObject) {
		...
		{
			DbgPrint("[ ProcLogger ] [ RegMonitor ] [ ERROR ] CmCallbackGetKeyObjectID : (%x)\n", Status);
			goto Exit;
		}
		...
		if (CompleteName->Length && CompleteName->Buffer) {
			...
			KeyPath.Buffer = ExAllocatePoolWithTag(NonPagedPool, KeyPath.MaximumLength, REG_TAG);

			if (!KeyPath.Buffer) {
				DbgPrint("[ ProcLogger ] [ RegMonitor ] [ Error ] ExAllocatePool() FAIL\n");
				goto Exit;
			}
...
Exit:
	...
	if (KeyPath.Buffer) {
		ExFreePoolWithTag(KeyPath.Buffer, REG_TAG);
	}
	return Matched;
}
...
NTSTATUS RegistryFilterCallback(
	...
	if (NotifyClass == RegNtPreCreateKeyEx || NotifyClass == RegNtPreOpenKeyEx)
	{
		...
		if (RegPreOpenKey(RegInformation->RootObject, RegInformation->CompleteName))
		{
			DbgPrint("[ ProcLogger ] [ RegMonitor ] Prevent Opening Handle\n");
...
{
	NTSTATUS Status = STATUS_SUCCESS;
	RtlInitUnicodeString(&g_GlobalContext.Altitude, L"370000");
	...
	if (!NT_SUCCESS(Status = CmRegisterCallbackEx(
		...
		DbgPrint("[ ProcLogger ] [ RegMonitor ] [ ERROR ] CmRegisterCallbackEx Failed : (%x)\n", Status);
		return Status;
	}
	else {
		DbgPrint("[ ProcLogger ] [ RegMonitor ] [ SUCCESS ] CmRegisterCallbackEx Success\n");
	}
...
VOID UnInstallRegMonitor()
{
	...
	if (!NT_SUCCESS(Status = CmUnRegisterCallback(g_GlobalContext.Cookie))) {
		DbgPrint("[ ProcLogger ] [ RegMonitor ] [ ERROR ] CmUnRegisterCallback Failed (%x)\n", Status);
		return Status;
	}
	else {
